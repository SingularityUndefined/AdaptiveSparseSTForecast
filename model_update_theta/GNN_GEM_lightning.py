...
class GEM_GNN(L.LightningModule):
    def __init__(self, num_nodes, num_neighbors, neighbor_list, mu, gamma, emb_dim=6, feature_dim=3, c=8, theta=0.5, method='CG', inv_method='L+eI', CG_iters=10, PGD_iters=100, PGD_step_size=0.01, scale=True, GEM_iters=5, lr=0.01, epsilon=0.2, update_theta=True):
        super().__init__()
        self.automatic_optimization = False  # 手动优化
        self.update_theta = update_theta # if backward the entire pipeline to update theta, or only backward the proxy loss to update w (theta fixed)
    ...
    def _r_tilde(self, w, CG_method='batch_PCG'):
        ...
        if self.method == 'CG':
            ...
            tilde_inv_ref = diag_inv_ref[:,None] + diag_inv_ref[None,:] - inv_ref - inv_ref.T  # (N, N), all (e_i - e_j)^T (L + J)^-1 (e_i - e_j) including non-edges
            #######################
            # inv_ref = z.unsqueeze(-1) @ Ainv_z.unsqueeze(1)  # (B, N, N)
            # # print(inv_ref.shape)
            # diag_inv_ref = torch.diagonal(inv_ref, dim1=1, dim2=2)  # (N,)
            # # print(diag_inv_ref.shape)

            # tilde_inv_ref = diag_inv_ref[:,:,None] + diag_inv_ref[:,None,:] - inv_ref - inv_ref.transpose(1,2)  # (N, N), all (e_i - e_j)^T (L + J)^-1 (e_i - e_j) including non-edges
            # tilde_inv_ref = tilde_inv_ref.mean(0)
    ...
    def estimate_r_tilde(self, w):
        pass  #TODO To be implemented: use LOBPCG to estimate r_tilde without explicitly computing the inverse, which can be more efficient for large graphs
    ...
    def on_train_epoch_end(self):
        if self.current_epoch % 4 == 0:
            ...
            draw_graph(
                ...
                filename=f"learned_graph_epoch_{self.current_epoch}_{self.method}.png"
            )
        return super().on_train_epoch_end()
