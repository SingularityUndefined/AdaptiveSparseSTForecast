...
Graph Learning Module
    - generate features from graph signals and embeddings
...
class CG_solver(nn.Module):
    def __init__(self, CG_iters, alpha_init, beta_init):
        super(CG_solver, self).__init__()
        self.alpha = nn.Parameter(torch.ones(CG_iters) * alpha_init)
        self.beta = nn.Parameter(torch.ones(CG_iters) * beta_init)
        self.CG_iters = CG_iters

    def forward(self, A_func, B_vecs, Minv_func=None, X0=None):
        if Minv_func is None:
            def Minv_func(x):
                return x
        ...
        for k in range(self.CG_iters):
            ...
            X = X + self.alpha[k] * P        # (B, N)
            R = R - self.alpha[k] * AP       # (B, N)

            Z = Minv_func(R)                 # (B, N)
            P = Z + self.beta[k] * P      # (B, N)        

        return X
    
class UnrolledGEM(nn.Module):
    def __init__(self,
                num_nodes, num_neighbors, neighbor_list, 
                mu, gamma, 
                emb_dim=6, feature_dim=3, c=8, theta=0.5, scale=True,
                method='CG', inv_method='L+J', 
                E_step_iters=3, inv_CG_iters=5, PGD_iters=10, 
                M1_step_size=0.01, PGD_step_size=0.01, 
                GEM_iters=5, 
        ...
        self.M1_iters = M1_iters # GD in M-step 1

        self.E_CG_iters = E_step_iters
        self.M1_CG_iters = inv_CG_iters
        self.M2_CG_iters = inv_CG_iters
        ...
        self.M1_step_size = M1_step_size
        self.PGD_step_size = PGD_step_size
        self.gamma = nn.Parameter(torch.ones((self.GEM_iters, self.PGD_iters)) * gamma)  # learnable gamma for M-step 2
        self.scale = scale
        ...
        ## TODO: introduce CG and PGD block
    ...
    def LHS_E_step(self, x, w, gem_iter):
        # solve equation (I + mu*L)X = Y
        return self.mu[gem_iter] * self.apply_L(x, w) + x # (B, N)
    
    def E_step(self, y, w, gem_iter):
        # y in (B, N)
        B = y.size(0)

        def A_func(x):
            return self.LHS_E_step(x, w, gem_iter)

        x = self.E_CG_solvers[gem_iter](A_func, y)
    ...
    def _r_tilde(self, w, CG_solver):
        ...
        if self.method == 'CG':
            ...
            inv_ref = CG_solver(A_func, e_input, Minv_func=Minv_func)
            ...
            inv_ref = torch.Tensor(factor(e_input.T.detach().cpu().numpy()).T).to(device)  # (N-1, N)
            ...
            Ainv_z = CG_solver(A_func, z, Minv_func=Minv_func)
            inv_ref = z.T @ Ainv_z / num_probes  # (N, N)
    ...
    def M_step_2(self, x, w_0, gem_iter, A_init=None):
        '''
        ...
        for iter in range(self.PGD_iters):
            ...
            # print(gem_iter, iter)
            r_tilde = self._r_tilde(w, self.M2_CG_solvers[gem_iter][int(iter)])  # (N, k)
            # PGD
            A_old = A.clone()
            A_new = A_old - self.PGD_step_size * w_0_alpha * (edge_diff_squares - r_tilde)
            A_new = self.soft_thresholding(A_new, tau=self.PGD_step_size * self.gamma[gem_iter, iter])
    ...
    def single_step(self, y, w_0, A, gem_iter, params=None):
        '''
        GEM framework:
        ...
        w, _ = self.scale_w(w)
        # E step
        x = self.E_step(y, w, gem_iter)  # (B, N)
        # update weight matrix 
        w_0 = self.glm(x, params=params)
        w, _ = self.scale_w(w_0 * A) # rescale

        # M step 1
        new_params = self.M_step_1(x, w, gem_iter)
        ...
        # M step 2
        A_new = self.M_step_2(x, w_0, gem_iter, A_init=A)
    ...
    def forward(self, y, w_init=None, A_init=None, params=None, epochs=1):
        ...
        for it in range(self.GEM_iters):
            ...
            x, w_0, A, new_params = self.single_step(y, w_0, A, it, params=params)
            size_A = A.size()
            w, _ = self.scale_w(w_0 * A)
            params = new_params
