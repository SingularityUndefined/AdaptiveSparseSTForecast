...
class Generalized_EM(nn.Module):
    ...
    def M_step_1_w_only(self, x, w, A):
        ...
        for i in range(self.M1_iters):
            ...
            w_update = detached_w - d_w / Lw    # MM step
            # compute metrics
            objective_new = self.objective(x, w_update)
            print(f'M-step 1 w-only iter {i+1}/{self.M1_iters}, objective: {objective_new:.4f} (old: {objective_old:.4f}), improvement: {objective_new - objective_old:.4f})')
    ...
    def M_step_2(self, x, w_0, A_init=None):
        ...
        A_new_select = torch.where(importance > 0.2, torch.ones_like(A), torch.zeros_like(A))  # (N, k)
        A_new_deleted = torch.where(importance < -0.2, torch.zeros_like(A), torch.ones_like(A))  # (N, k)
        A_new = (A + A_new_select > 0) * A_new_deleted  # keep edges with importance > 0.2, remove edges with importance < -0.2, keep others unchanged
        ...
        w_new = w_0_alpha * A_new
        objective_new = self.objective(x, w_new)
        print(f'M-step 2 edge selection, objective: {objective_new:.4f} (old: {objective_old:.4f}), improvement: {objective_new - objective_old:.4f})')
        return A_new
