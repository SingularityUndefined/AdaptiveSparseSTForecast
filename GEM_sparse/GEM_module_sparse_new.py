...
class Generalized_EM(nn.Module):
    def __init__(self, num_nodes, num_neighbors, neighbor_list, mu, gamma, step_size, emb_dim=6, feature_dim=3, c=8, theta=0.5, method='CG', inv_method='L+eI', use_proxy_loss=True, CG_iters=10, PGD_iters=100, PGD_step_size=0.01, scale=True, GEM_iters=5, epsilon=0.2, M1_iters=5, update_theta=True, e_M2=1.0, MM_step1=False, MM_step2=False, xi=1e-2, tau=0.2):
        ...
        self.scale = scale
        self.method = method
        assert method in ['CG', 'chebyshev', 'cholmod', 'Hutchinson', 'edge_CG', 'edge_cholmod', 'selected_cholmod'], "Only 'CG', 'chebyshev', 'cholmod', 'Hutchinson', 'edge_CG', 'edge_cholmod' and 'selected_cholmod' are supported currently."
    ...
    def _selected_inverse_from_cholmod_factor(self, factor):
        """
        Compute selected entries of A^{-1} on the Cholesky fill pattern.
        CHOLMOD returns L such that A[P, P] = L @ L.T.
        The returned dictionary stores entries of inv(A[P, P]) as keys (i, j), i <= j.
        """
        L = factor.L().tocsc()
        n = L.shape[0]
        columns = []
        ...
        for col in range(n):
            start, end = L.indptr[col], L.indptr[col + 1]
            rows = L.indices[start:end]
            values = L.data[start:end]
            diag_values = values[rows == col]
            if diag_values.size != 1:
                raise ValueError("Invalid Cholesky factor: missing or repeated diagonal entry.")
            below_mask = rows > col
            columns.append((diag_values[0], rows[below_mask], values[below_mask]))

        selected = {}
        for col in range(n - 1, -1, -1):
            diag, below_rows, below_values = columns[col]

            for row in below_rows:
                total = 0.0
                for k, l_ki in zip(below_rows, below_values):
                    key = (k, row) if k <= row else (row, k)
                    total += l_ki * selected[key]
                selected[(col, row)] = -total / diag

            total = 0.0
            for row, l_ri in zip(below_rows, below_values):
                total += l_ri * selected[(col, row)]
            selected[(col, col)] = 1.0 / (diag * diag) - total / diag
    ...
    def _r_tilde(self, w, CG_method='batch_PCG'):
        ...
        if self.method == 'CG':
            ...
            sparse_L = self.sparse_L_from_w(detached_w).tocsc()

            if self.inv_method == 'L+J':
                # For b = e_i - e_0, b has zero sum. Solving with (L + J)^{-1}
                # is equivalent to solving the reduced Laplacian with node 0 grounded.
                factor = cholesky(sparse_L[1:, 1:].tocsc())
                rhs = np.eye(N - 1, dtype=np.float64)
                reduced_sol = factor(rhs).T  # (N-1, N-1), row i-1 solves e_i - e_0
                inv_ref_np = np.zeros((N - 1, N), dtype=np.float64)
                inv_ref_np[:, 1:] = reduced_sol
            elif self.inv_method == 'L+eI':
                L_plus_epsilon_I = sparse_L + self.epsilon * sp.eye(N, format='csc')
                factor = cholesky(L_plus_epsilon_I)
                inv_ref_np = factor(e_input.T.detach().cpu().numpy()).T  # (N-1, N)
            else:
                raise ValueError(f"Unsupported inv_method: {self.inv_method}")

            inv_ref = torch.as_tensor(inv_ref_np, device=w.device, dtype=w.dtype)
            inv_ref = torch.concat([torch.zeros((1, N), device=w.device), inv_ref], dim=0)
        ...
        elif self.method == 'edge_CG':
            # Simple baseline: solve A y = e_i - e_j directly for candidate edges.
            # This avoids forming an NxN inverse approximation, but is usually slower
            # than solving N reference RHS when k is not tiny.
            device = w.device
            dtype = w.dtype
            flat_mask = self.neighbor_mask.to(device).view(-1)
            edge_rows = torch.arange(N, device=device).unsqueeze(1).repeat(1, k).view(-1)[flat_mask]
            edge_cols = self.neighbor_list.to(device).view(-1)[flat_mask]
            edge_count = edge_rows.numel()
            r_values = torch.empty(edge_count, device=device, dtype=dtype)
            chunk_size = min(1024, max(1, edge_count))

            def A_func(x):
                if self.inv_method == 'L+J':
                    return self.apply_L_plus_J(x, w)
                elif self.inv_method == 'L+eI':
                    return self.apply_L_plus_epsilon_I(x, w)

            def Minv_func(x):
                if self.inv_method == 'L+J':
                    return self.apply_inv_D_plus_J(x, w)
                elif self.inv_method == 'L+eI':
                    return self.apply_inv_D(x, w)

            for start in range(0, edge_count, chunk_size):
                end = min(start + chunk_size, edge_count)
                rows = edge_rows[start:end]
                cols = edge_cols[start:end]
                B_edges = torch.zeros((end - start, N), device=device, dtype=dtype)
                idx = torch.arange(end - start, device=device)
                B_edges[idx, rows] = 1.0
                B_edges[idx, cols] -= 1.0

                if CG_method == 'batch_CG':
                    sol, _ = batch_CG(A_func, B_edges, tol=1e-6, max_iter=self.CG_iters)
                elif CG_method == 'batch_PCG':
                    sol, _ = batch_PCG(A_func, B_edges, Minv_func=Minv_func, tol=1e-6, max_iter=self.CG_iters)
                else:
                    raise ValueError("Only 'batch_CG' and 'batch_PCG' are supported currently.")

                r_values[start:end] = torch.sum(B_edges * sol, dim=1)

            r_tilde = torch.zeros((N, k), device=device, dtype=dtype)
            r_tilde.view(-1)[flat_mask] = r_values
            return r_tilde

        elif self.method == 'edge_cholmod':
            # Exact baseline with sparse Cholesky, extracting only candidate-edge
            # quadratic forms instead of materializing the full inverse.
            device = w.device
            dtype = w.dtype
            flat_mask = self.neighbor_mask.to(device).view(-1)
            edge_rows = torch.arange(N, device=device).unsqueeze(1).repeat(1, k).view(-1)[flat_mask]
            edge_cols = self.neighbor_list.to(device).view(-1)[flat_mask]
            edge_count = edge_rows.numel()
            r_values_np = np.empty(edge_count, dtype=np.float64)
            chunk_size = min(1024, max(1, edge_count))
            ...
            if self.inv_method == 'L+J':
                ...
                factor = cholesky(sparse_L + self.epsilon * sp.eye(N, format='csc'))
            else:
                raise ValueError(f"Unsupported inv_method: {self.inv_method}")

            edge_rows_np = edge_rows.detach().cpu().numpy()
            edge_cols_np = edge_cols.detach().cpu().numpy()
            for start in range(0, edge_count, chunk_size):
                end = min(start + chunk_size, edge_count)
                rows = edge_rows_np[start:end]
                cols = edge_cols_np[start:end]
                width = end - start

                if self.inv_method == 'L+J':
                    rhs = np.zeros((N - 1, width), dtype=np.float64)
                    col_idx = np.arange(width)
                    non_root_rows = rows != 0
                    non_root_cols = cols != 0
                    rhs[rows[non_root_rows] - 1, col_idx[non_root_rows]] += 1.0
                    rhs[cols[non_root_cols] - 1, col_idx[non_root_cols]] -= 1.0
                else:
                    rhs = np.zeros((N, width), dtype=np.float64)
                    col_idx = np.arange(width)
                    rhs[rows, col_idx] += 1.0
                    rhs[cols, col_idx] -= 1.0

                sol = factor(rhs)
                r_values_np[start:end] = np.sum(rhs * sol, axis=0)

            r_tilde = torch.zeros((N, k), device=device, dtype=dtype)
            r_tilde.view(-1)[flat_mask] = torch.as_tensor(r_values_np, device=device, dtype=dtype)
            return r_tilde

        elif self.method == 'selected_cholmod':
            # Exact selected inverse on the Cholesky fill pattern via Takahashi recursion.
            # This extracts diagonal and candidate-edge inverse entries without solving
            # one RHS per edge and without materializing a dense inverse.
            device = w.device
            dtype = w.dtype
            flat_mask = self.neighbor_mask.to(device).view(-1)
            edge_rows = torch.arange(N, device=device).unsqueeze(1).repeat(1, k).view(-1)[flat_mask]
            ...
            edge_rows_np = edge_rows.detach().cpu().numpy()
            edge_cols_np = edge_cols.detach().cpu().numpy()
            r_values_np = np.empty(edge_rows_np.shape[0], dtype=np.float64)

            sparse_L = self.sparse_L_from_w(w.detach().cpu()).tocsc()
            if self.inv_method == 'L+J':
                factor = cholesky(sparse_L[1:, 1:].tocsc())
                selected_inv = self._selected_inverse_from_cholmod_factor(factor)
                perm = factor.P()
                inv_perm = np.empty_like(perm)
                inv_perm[perm] = np.arange(perm.size)

                def get_reduced_inv(original_row, original_col):
                    row = inv_perm[original_row - 1]
                    col = inv_perm[original_col - 1]
                    key = (row, col) if row <= col else (col, row)
                    return selected_inv[key]

                for idx, (row, col) in enumerate(zip(edge_rows_np, edge_cols_np)):
                    if row == 0:
                        r_values_np[idx] = get_reduced_inv(col, col)
                    elif col == 0:
                        r_values_np[idx] = get_reduced_inv(row, row)
                    else:
                        r_values_np[idx] = (
                            get_reduced_inv(row, row)
                            + get_reduced_inv(col, col)
                            - 2.0 * get_reduced_inv(row, col)
                        )
            elif self.inv_method == 'L+eI':
                factor = cholesky(sparse_L + self.epsilon * sp.eye(N, format='csc'))
                selected_inv = self._selected_inverse_from_cholmod_factor(factor)
                ...
                def get_inv(original_row, original_col):
                    row = inv_perm[original_row]
                    col = inv_perm[original_col]
                    key = (row, col) if row <= col else (col, row)
                    return selected_inv[key]
    ...
    def M_step_1_w_only(self, x, w, A, GD=True):
        ...
        for i in range(self.M1_iters):
            ...
            if GD:
                w_update = torch.clamp(detached_w - self.step_size * d_w, min=0.0)  # gradient step
            # TODO: alternativew_update, MM
            else:
    ...
    def M_step_1_proxy_loss(self, x, w):
        ...
        if self.use_proxy_loss:
            ...
            proxy_loss = self.GLR(x, w) - (r_tilde * w).sum() / 2 + self.gamma * torch.log(1 + w/self.xi).sum() / 2  # each edge counted twice
            if self.MM_step1:
                # Lipschitz constant
                Lw = (math.sqrt(N) + 1) ** 2 / self.epsilon ** 2   # (1/mu * ||L||_2 + epsilon)^2
                print('Lipschitz constant for w:', Lw)
    ...
    def M_step_2(self, x, w_0, A_init=None):
        ...
        importance = torch.log(1 - w * r_tilde) + edge_diff_squares * w + self.gamma * torch.log(1 + w / self.xi)  # (N, k)
        A_cut = importance > self.tau
        A_new = torch.where(A_cut, torch.zeros_like(A), A)  # remove edges with positive importance, keep others unchanged
    ...
    def single_step(self, y, w_0, A, params=None):
        ...
        if self.update_theta:
            ...
            w_0 = self.M_step_1_w_only(x, w, A)  # update w_0 only on MM
            new_params = None
    ...
    def single_step_GLASSO(self, y, w_0, A, params=None):
        ...
        glasso = GraphicalLasso(alpha=self.gamma, max_iter=self.M1_iters)
        ...
        A_new = torch.where(w_new > 0, torch.ones_like(w_new), torch.zeros_like(w_new))  # binary mask from glasso result
        # get glasso w
        # mask laplacian with A to get the observed entries

        return x, w_new, A_new
    
    def single_step_MM(self, y, w_0, A, params=None):
        w = w_0 * A  # (N, k)
        w, _ = self.scale_w(w)
        # E step
        x = self.E_step(y, w)  # (B, N)
        # update weight with MM-based alternative GLASSO
        # for i in range(20):
            # print(f'  MM Iter {i+1}/{20}')
            # self.M1_iters = 1  # only one GD step for w update in each MM iteration
        w_new = self.M_step_1_w_only(x, w, A, GD=True)  # update w only on MM
        A_new = w_new > 0  # binary mask from MM result
        A_new = A_new.float() * self.neighbor_mask.float()  # mask non-edges
        w = w_new * A_new  # update w with new mask
        A = A_new

        return x, w_new, A_new
    
    def forward(self, y, w_init=None, A_init=None, params=None, epochs=1, w_GT=None, use_single_M_step=False, single_step_type='MM', graph_type='circular'):
    
        # init w and A
        assert single_step_type in ['GLASSO', 'MM'], "single_step_type must be either 'GLASSO' or 'MM'"
        ...
        for it in range(self.GEM_iters):
            ...
            if not use_single_M_step:
                x, w_0, A, new_params = self.single_step(y, w_0, A, params=params)
                size_A = A.size()
                w, _ = self.scale_w(w_0 * A)
                params = new_params
            else:
                if single_step_type == 'GLASSO':
                    x, w_0, A = self.single_step_GLASSO(y, w_0, A, params=params)
                elif single_step_type == 'MM':

                    x, w_0, A = self.single_step_MM(y, w_0, A, params=params)
                size_A = A.size()
                w, _ = self.scale_w(w_0 * A)
